...
bookers = bookers[bookers.visited_w2v_clusters.apply(lambda tl: len(tl) >= 4)]
non_bookers = non_bookers[non_bookers.visited_w2v_clusters.apply(lambda tl: len(tl) >= 4)]
...
displayHTML('<p>Based on <code>{}</code>, the hypothesis <i>"Bookers converge more than non-bookers"</i> is <b>{}</b> (p-value: {:.4f}).</p>'.format(
            'w2v_cluster_change', bookers_converge_more(bookers, non_bookers, 'w2v_cluster_change'), w2v_p_value))